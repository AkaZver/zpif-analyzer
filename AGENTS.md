...
- `internal/llm/` — LLM интеграция (OpenAI-совместимые API)
  - `client.go` — HTTP клиент для LLM API
...
3. **Excel экспорт** — использует библиотеку excelize с форматированием:
...
6. **Рыночные данные** — автоматическая загрузка с MOEX ISS API, investfunds.ru и vsezpif.ru
