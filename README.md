...
- **Автоматическая загрузка данных**: Интеграция с MOEX ISS API, investfunds.ru и vsezpif.ru для получения рыночных данных
...
- **LLM-анализ**: Анализ документов с извлечением метрик и оценкой рисков (без обновления финансовых данных)
...
Проект поддерживает работу с любыми OpenAI-совместимыми API (OpenAI, OpenRouter, Anthropic, Ollama и др.).

### Особенности обработки ответов LLM

Для надёжного парсинга ответов от различных LLM моделей реализованы специальные функции:

- **ExtractJSON** — извлечение JSON из ответа LLM:
  - Удаление BOM (byte order mark)
  - Извлечение JSON из markdown code blocks (` ```json ... ``` `)
  - Замена умных двойных кавычек (`""`) на обычные двойные кавычки
  - Замена умных одинарных кавычек (`''`) на обычные одинарные кавычки
  - Поиск сбалансированного JSON-объекта с учётом строк и escape-последовательностей

- **SanitizeJSON** — исправление невалидного JSON:
  - Замена одинарных кавычек на двойные вне строковых значений (для поддержки JSON с одинарными кавычками как разделителями)
  - Замена неэкранированных двойных кавычек внутри строковых значений на одинарные кавычки
  - Сохранение валидных escape-последовательностей (`\"`)

### Часовой пояс

Имена файлов документов создаются с использованием московского времени (Europe/Moscow, GMT+3) для удобства пользователей.

### Таймауты

- HTTP-клиент LLM: 180 секунд
- Контекст для операций поиска: 180 секунд
...
- `GET /api/funds-with-financials` - Список фондов с последними финансовыми данными (оптимизированный для Dashboard)
- `GET /api/funds/:id` - Детали фонда
- `POST /api/funds` - Создать фонд
- `PUT /api/funds/:id` - Обновить фонд
- `DELETE /api/funds/:id` - Удалить фонд (hard delete с каскадным удалением всех связанных данных)
...
### Экспорт
- `GET /api/export/excel` - Экспорт в Excel (один лист "Фонды" с 15 колонками)

### Рыночные данные
- `POST /api/funds/:id/fetch-market-data` - Загрузить рыночные данные для фонда (MOEX + investfunds + vsezpif)
...
### vsezpif.ru
Дополнительные данные о фондах: комиссия УК, количество объектов, сегмент недвижимости.

**Особенности:**
- Поиск фонда по ISIN
- Извлечение метрик из JSON API
- Автоматическое заполнение полей фонда
...
### Фильтрация будущих дат
Система автоматически фильтрует данные с датами в будущем на всех уровнях:
- **Парсеры**: данные с будущими датами не сохраняются при загрузке
- **Сервисы**: проверка дат перед созданием записей в БД
- **Репозитории**: SQL запросы включают условие `snapshot_date <= NOW()`

Это предотвращает появление некорректных данных на графиках и в расчетах.
