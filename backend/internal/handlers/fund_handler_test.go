...
func TestFundHandler_GetAllFundsWithLatestFinancials_Success(t *testing.T) {
	handler, router, mock, cleanup := setupTestFundHandler(t)
	defer cleanup()
	...
	req := httptest.NewRequest("GET", "/api/funds-with-financials", nil)
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)
...
func TestFundHandler_GetAllFundsWithLatestFinancials_ServiceError(t *testing.T) {
	handler, router, mock, cleanup := setupTestFundHandler(t)
	defer cleanup()
